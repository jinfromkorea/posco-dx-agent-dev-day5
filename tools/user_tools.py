...
def _biz_name(entry) -> str:
    """'업무용' 항목에서 시스템 이름을 반환합니다 (문자열 또는 {name, shared_account} 객체)."""
    return entry["name"] if isinstance(entry, dict) else entry


def _biz_shared_account(entry) -> str:
    """업무용 항목의 shared_account ID를 반환합니다. 없으면 빈 문자열."""
    return entry.get("shared_account", "") if isinstance(entry, dict) else ""
...
async def get_my_systems(config: RunnableConfig) -> str:
    ...
    # 업무용: 문자열 또는 {name, shared_account} 객체 리스트
    if sys_map["업무용"]:
        lines.append("\n**[업무용]** (EP SSO 통합 로그인)")
        for entry in sys_map["업무용"]:
            name = _biz_name(entry)
            shared_id = _biz_shared_account(entry)
            sys_info = next((s for s in _systems if s["name"] == name), None)
            brief = _format_system_brief(sys_info) if sys_info else f"• **{name}** (시스템 정보 없음)"
            if shared_id:
                brief += f" | 🔑 공용계정: `{shared_id}`"
            lines.append(brief)
...
async def grant_system_access(
    ...
    Args:
        ...
        account: 개인용 시 사용하는 계정명
        shared_account: 업무용 공용계정 시스템의 shared_accounts.yaml ID (예: jenkins_poscodx)
    """
    ...
    sys_map = _get_system_map(user)
    all_names = [_biz_name(e) for e in sys_map["업무용"]] + [_personal_name(e) for e in sys_map["개인용"]]
    ...
    if use_type == "개인용":
        ...
        if shared_account:
            sys_map["업무용"].append({"name": actual_name, "shared_account": shared_account})
        else:
...
async def revoke_system_access(user_id: str, system_name: str) -> str:
    ...
    matched = None
    matched_type = None
    # 업무용: 문자열 또는 {name, shared_account} 객체
    found_biz = next((e for e in sys_map["업무용"] if system_name.lower() in _biz_name(e).lower()), None)
    if found_biz:
    ...
    removed_name = _personal_name(matched) if matched_type == "개인용" else _biz_name(matched)
