...
def _get_system_map(user: dict) -> dict[str, list[str]]:
    """systems 필드를 {업무용: [...], 개인용: [...]} 형태로 반환합니다.
...
async def get_my_systems(config: RunnableConfig) -> str:
    ...
    for use_type, names in sys_map.items():
        if not names:
            continue
        lines.append(f"\n**[{use_type}]**")
        for name in names:
...
@tool
async def grant_system_access(user_id: str, system_name: str, use_type: str = "업무용") -> str:
    ...
    sys_map = _get_system_map(user)
    if actual_name in sys_map["업무용"] or actual_name in sys_map["개인용"]:
        return f"'{user['name']}' 님은 이미 '{actual_name}' 시스템에 접근 권한이 있습니다."

    if use_type not in ("업무용", "개인용"):
        use_type = "업무용"
    sys_map[use_type].append(actual_name)
    user["systems"] = sys_map
    await _save_users(users)

    return f"✅ '{user['name']}' ({user_id}) 님에게 '{actual_name}' 접근 권한이 부여되었습니다. ({use_type})"
...
async def revoke_system_access(user_id: str, system_name: str) -> str:
    ...
    matched_type = None
    for use_type, names in sys_map.items():
        found = next((s for s in names if system_name.lower() in s.lower()), None)
        if found:
            matched = found
            matched_type = use_type
            break
    if not matched:
    ...
    return f"✅ '{user['name']}' ({user_id}) 님의 '{matched}' 접근 권한이 취소되었습니다."
